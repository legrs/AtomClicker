...
let disSelect = document.querySelector('#disSelect'); 
...
let developerName =    ['質量エネルギー化装置','粒子合成装置','原子加速装置','原子分解装置','原子中性子化装置'];
let developerArray=    ['toEnergy',       'toAtom',   'atomKsk',  'toPNE',    'toNeutron'];
let upgradedDeveloper = [0,0,0,0,0]; 
let maxUpgradeDeveloper=[3,1,2,1,1];
...
let developerNeedEnergy= [0.1,1,50,30,1];
...
function processingDeveloper(element){ //開発の処理
    ...
    if(upgradedDeveloper[developerArray.indexOf(element)] != 0){ //作成済み
        if(developerInnerAtom[developerArray.indexOf(element)][0] != ''){ //中身あり
            switch(element){
                case 'toEnergy':
                    energy += (developerNeedEnergy[developerArray.indexOf(element)] * (upgradedDeveloper[developerArray.indexOf(element)])) * developerInnerAtomQty[developerArray.indexOf(element)][0];
                    
                    developerInnerAtom[developerArray.indexOf(element)][0] = '';
                    developerInnerAtomQty[developerArray.indexOf(element)][0] = 0;
                    developerInfo(element);
                    break;
                case 'toAtom':
                    if(developerInnerAtom[developerArray.indexOf(element)].includes('p') && developerInnerAtom[developerArray.indexOf(element)].includes('e')){ 
                        if(developerInnerAtomQty[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('p')] / developerInnerAtomQty[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('e')] == 1 && developerInnerAtom[developerArray.indexOf(element)].indexOf('n')   == -1){
                            HQty += developerInnerAtomQty[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('p')];
                            console.log(developerInnerAtomQty[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('p')]);
                            energy -= developerNeedEnergy[developerArray.indexOf(element)] * (upgradedDeveloper[developerArray.indexOf(element)]) * SUMArray(developerInnerAtomQty[developerArray.indexOf(element)]);
                            
                            developerInnerAtomQty[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('p')] = 0;
                            developerInnerAtomQty[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('e')] = 0;
                            developerInnerAtom[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('p')] = '';
                            developerInnerAtom[developerArray.indexOf(element)][developerInnerAtom[developerArray.indexOf(element)].indexOf('e')] = '';
                            while(developerInnerAtom[developerArray.indexOf(element)].length != 1){
                                developerInnerAtom[developerArray.indexOf(element)].pop();
                            }
                            while(developerInnerAtomQty[developerArray.indexOf(element)].length != 1){
                                developerInnerAtomQty[developerArray.indexOf(element)].pop();
                            }
                            developerInfo(element);
                        }else{
                            switch(`${GCD(developerInnerAtom[developerArray.indexOf(element)].includes('p'),developerInnerAtom[developerArray.indexOf(element)].includes('n'),developerInnerAtom[developerArray.indexOf(element)].includes('e'))}`){
                                case ``:
                                    break;
                            }
                        }
...
function autoAtomClick(element){
    if(autoAtomArray.includes(element.target.id)){ //アップグレードがクリックされていた時   番号→autoAtomArray.indexOf(element.target.id)
        ...
        if(canLevelUp == autoAtomcost[autoAtomArray.indexOf(element.target.id)].length){ //LevelUp可能だった時
            ...
            autoAtomQtyHTMLArray[autoAtomArray.indexOf(element.target.id)].innerHTML = toSI(autoAtomQtyArray[autoAtomArray.indexOf(element.target.id)]);
...
function selectDev(element){  //developerが左クリック↓〃
    let canLevelUp = false;
    console.log(HQty);
    if(developerArray.includes(element.target.id)){   //番号→developerArray.indexOf(element.target.id)
        if(developerInnerAtom[developerArray.indexOf(element.target.id)][0] == ''){ //中身無し,クリック 
            if(selectAtom == ''){  //元素を選択していない、中身無し,クリック,つまりレベルアップ
                console.log('levelUp??');
                for(let i=0;developerNeedAtom[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)].length > i;i++){
                switch(developerNeedAtom[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i]){
                    case 'H':
                        if(developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i] <= HQty){
                            HQty = HQty - developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i];
                            upgradedDeveloper[developerArray.indexOf(element.target.id)]++;
                            canLevelUp = true;
                        }
                        break;
                    case 'He':
                        if(developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i] <= HeQty){
                            HeQty = HeQty - developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i];
                            upgradedDeveloper[developerArray.indexOf(element.target.id)]++;
                            canLevelUp = true;
                        }
                        break;
                    case 'Li':
                        if(developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i] <= LiQty){
                            LiQty = LiQty - developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i];
                            upgradedDeveloper[developerArray.indexOf(element.target.id)]++;
                            canLevelUp = true;
                        }
                        break;
                    case 'Be':
                        if(developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i] <= BeQty){
                            BeQty = BeQty - developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i];
                            upgradedDeveloper[developerArray.indexOf(element.target.id)]++;
                            canLevelUp = true;
                        }
                        break;
                    case 'B':
                        if(developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i] <= BQty){
                            BQty = BQty - developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element.target.id)]][developerArray.indexOf(element.target.id)][i];
                            upgradedDeveloper[developerArray.indexOf(element.target.id)]++;
                            canLevelUp = true;
                        }
                        break;
            ...
            if(toridasi == 'ON'){
                console.log('取り出し');
                for(let i=0;i < developerInnerAtom[developerArray.indexOf(element.target.id)].length;i++){
                    switch(developerInnerAtom[developerArray.indexOf(element.target.id)][i]){
                        case 'H':
                                HQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'He':
                                HeQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'Li':
                                LiQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'Be':
                                BeQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'B':
                                BQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'C':
                                CQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'N':
                                NQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'O':
                                OQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'F':
                                FQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                        case 'Ne':
                                NeQty += developerInnerAtomQty[developerArray.indexOf(element.target.id)][i];
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][i]=0;
                            break;
                    }
                    developerInnerAtom[developerArray.indexOf(element.target.id)][i] = '';
                }
            }
        }
        if(upgradedDeveloper[developerArray.indexOf(element.target.id)] != 0){ //作成済み
            //投入↓
            if(toridasi == 'OFF'){
                let canPut = false;
                if(element.target.id == 'toAtom'){
                    switch(selectAtom){
                        case 'p':
                            if(proton >= addQty){
                                proton = proton - addQty;
                                canPut = true;
                            }
                            break;
                        case 'n':
                            if(neutron >= addQty){
                                neutron = neutron - addQty;
                                canPut = true;
                            }
                            break;
                        case 'e':
                            if(electron >= addQty){
                                electron = electron - addQty;
                                canPut = true;
                            }
                            break;
                    }
                    if(developerInnerAtom[developerArray.indexOf(element.target.id)][0] == ''){ //まだ0番目二元素が入っていないとき
                        if(canPut == true){
                            addAtom();
                            console.log('投入成功');
                            developerInnerAtom[developerArray.indexOf(element.target.id)][0] = selectAtom;
                            developerInnerAtomQty[developerArray.indexOf(element.target.id)][0] = developerInnerAtomQty[developerArray.indexOf(element.target.id)][0]+addQty;
                            console.log(element.target.id);
                            developerInfo(element.target.id);
                        }
                    }else{
                        ...
                        if(canPut == true){
                            ...
                            console.log('投入成功');
                            for(let i=0;i < developerInnerAtom[developerArray.indexOf(element.target.id)].length;i++){
                                if(developerInnerAtom[developerArray.indexOf(element.target.id)][i] == selectAtom){
                                    maetoOnaji = developerInnerAtom[developerArray.indexOf(element.target.id)].indexOf(selectAtom);
                                }
                            }
                            if(maetoOnaji != -100){
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][maetoOnaji] = developerInnerAtomQty[developerArray.indexOf(element.target.id)][maetoOnaji] + addQty;
                            }else{
                                developerInnerAtom[developerArray.indexOf(element.target.id)].push(selectAtom);
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)].push(addQty);
                            }
                            console.log(element.target.id);
                            developerInfo(element.target.id);
                        }
                    ...
                    if(element.target.id == 'toEnergy'){
                        ...
                        if(developerInnerAtom[developerArray.indexOf(element.target.id)][0] == ''){ //まだ0番目二元素が入っていないとき
                            ...
                            if(canPut == true){
                                addAtom();
                                console.log('投入成功！');
                                developerInnerAtomQty[developerArray.indexOf(element.target.id)][0] += addQty;
                                console.log(element.target.id);
                                developerInfo(element.target.id);
                            }
                        }
                        ...
                        if(developerInnerAtom[developerArray.indexOf(element.target.id)][0] == ''){ //まだ0番目二元素が入っていないとき
                            ...
                            if(canPut == true){
                                ...
                                for(let i=0;i < developerInnerAtom[developerArray.indexOf(element.target.id)].length;i++){
                                    if(developerInnerAtom[developerArray.indexOf(element.target.id)][i] == selectAtom){
                                        maetoOnaji = developerInnerAtom[developerArray.indexOf(element.target.id)][i].indexOf(selectAtom);
                                    }
                                }
                                if(maetoOnaji != -100){
                                    developerInnerAtomQty[developerArray.indexOf(element.target.id)][maetoOnaji] = developerInnerAtomQty[developerArray.indexOf(element.target.id)][maetoOnaji] + addQty;
...
function developerInfo(element){ //開発の説明出す
    ...
    if( developerInnerAtom[developerArray.indexOf(element)] == ''){ //中身がない時
        if(upgradedDeveloper[developerArray.indexOf(element)] == 0){ //作成済みでないとき
            ...
            for(let i = 0;i<developerNeedAtom[upgradedDeveloper[developerArray.indexOf(element)]][developerArray.indexOf(element)].length;i++){
                createNeedAtom = `${createNeedAtom} ${developerNeedAtom[upgradedDeveloper[developerArray.indexOf(element)]][developerArray.indexOf(element)][i]} ${developerNeedAtomQty[upgradedDeveloper[developerArray.indexOf(element)]][developerArray.indexOf(element)][i]},`;
            }
            info4.innerHTML = `左クリックで作成`;
        ...
        if(element != 'toEnergy'){
            info7.innerHTML = `${developerNeedEnergy[developerArray.indexOf(element)] / (upgradedDeveloper[developerArray.indexOf(element)]) * SUMArray(developerInnerAtomQty[developerArray.indexOf(element)])}E 消費`;
        }else{
            info7.innerHTML = `${toSI(Math.round((developerNeedEnergy[0] * (upgradedDeveloper[0])) * developerInnerAtomQty[0][0]))}E 生産`;
        }
    }
...
function hoverInfoON(element){
    switch(element.target.id){
        ...
        case 'disSelect':
            hoverInfo.style.display = 'block';
            name.innerHTML = '選択解除';
            info1.innerHTML = '元素,粒子の選択を解除します';
            info2.innerHTML = '';
            info3.innerHTML = ``;
            info4.innerHTML = '';
            info5.innerHTML = '';
...
disSelect.addEventListener('click',()=>{
    selectAtom = '';
    atomselect(-100);
    pneselect(-100);
});
...
setInterval(()=>{
    ...
    for(let i=0;i<5;i++){
        for(let f=0;i<developerInnerAtom[i].length;i++){
            if(developerInnerAtom[i][f] == ''){  //ナシ
                if(developerInnerAtomQty[i][f] != 0){ //ナシ,ナシ
                    developerInnerAtom[i][f] = '';
                    developerInnerAtomQty[i][f] = 0;
                }
            }else{ //アリ
                if(developerInnerAtomQty[i][f] == 0){ //アリ,ナシ//////
                    developerInnerAtom[i][f] = '';
                    developerInnerAtomQty[i][f] = 0;
                }
            }
